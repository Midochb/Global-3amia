...
const wFlagEl = document.getElementById("w_flag");
...
function normalizeRow(r){
  ...
  // Keep in sync with /recherche normalization so synonyms are reciprocal.
  // Some exports use `fr`, others `traduction_fr`, others `traduction`.
...
function renderWord(row, rows, synIndex){
  ...
  if(wFlagEl) {
    wFlagEl.textContent = flag;
    const cc = (row.pays_code || "").toString().trim().toUpperCase();
    wFlagEl.title = cc ? cc : "";
  }
